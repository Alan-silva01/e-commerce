...
const StoreHeroWidget = ({ data }: { data?: StoreData }) => {
  const store = data
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 2. Update store metadata with the uploaded image URL
      const updateRes = await fetch(`/admin/stores/${store.id}`, {
        method: "POST",
        headers: {
      ...
      if (!updateRes.ok) {
        throw new Error("Falha ao salvar metadados da loja.")
      }
  ...
  const handleRemoveImage = async () => {
    try {
      ...
      setHeroImage("")
      toast.success("Sucesso", {
        description: "Imagem do Hero removida da loja.",
      })
    } catch (error: any) {
      toast.error("Erro", {
      ...
      <div className="flex items-center justify-between pb-4">
        <div>
          ...
          <p className="text-xs text-ui-fg-subtle mt-1">
            Faça upload da imagem que aparecerá no topo da página inicial da sua loja.
          </p>
        </div>
...
export default StoreHeroWidget
